...
[boehm00]: http://goo.gl/kJE87M "Barry W. Boehm, Clark, Horowitz,
Brown, Reifer, Chulani, Ray Madachy, and Bert Steece. 2000. Software
Cost Estimation with Cocomo II (1st ed.). Prentice Hall"

The space of legal values for a COCOMO project  looks like this:
"""
data = dict(
  cols=[id, 
center,
...
'<effort',  # development effort in months (one month =152 hours. includes development+ management)
...
def mutate1(a,b,c):
    tmp= num(a+(b-c)/2)
    return 0 if tmp < 0 else int(round(tmp))

def mutate(all):
  return [mutate1(a,b,c) for a,b,c 
          in zip( any(all),any(all),any(all))]

def constrain(data,constraints):
  tmp = mutate(data.rows)
  for con in constraits:
    for key,lst in con.items():
      for i,x in enumerate(data.cols): 
      #XXXX get the index and key and rest the value 
...
if __name__ == "__main__":
    ...
    print(r["team"]["site"])
    for i in range(0):
        d = one( Coc2tunings,  [relaxSchedule(), flight()] )
        d["effort"] =COCOMO2effort(d) 
        print(d)
